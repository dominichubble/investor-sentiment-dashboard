...
\lstset{
    ...
    columns=fullflexible,   % Honour natural character widths in monospace font
    keepspaces=true,        % Preserve all literal whitespace (indentation, alignment)
    showstringspaces=false  % Do not render visible space-glyph in string literals
}
...
% Remove forced page break before every chapter (supervisor requested less whitespace
% between chapters). Chapters still render with their heading; they just flow
% continuously rather than starting on a new page.
\titleclass{\chapter}{straight}
\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries}{\chaptertitlename\ \thechapter}{14pt}{\Huge}
\titlespacing*{\chapter}{0pt}{2.0ex plus 1ex minus .2ex}{2.3ex plus .2ex}
\usepackage{natbib}
\usepackage{float}
\usepackage{tikz}
...
% Using \input (not \include) so chapters flow continuously without a forced
% \clearpage between them; combined with \titleclass{\chapter}{straight} above,
% this removes the blank half-pages that previously separated chapters.
\input{chapters/01_Introduction}
\input{chapters/02_LiteratureReview}
\input{chapters/03_TechnicalBackground}
\input{chapters/04_SystemDesignMethodology}
