...
\midrule
1 & Multi-source ingestion with scheduling and rate limits & Met & Chs.~\ref{chap:system_design}--\ref{chap:implementation}; Fig.~\ref{fig:system_architecture}; App.~\ref{app:env_table}. \\
...
Ingestion runs from three distinct source feeds into a shared database schema; FinBERT labels each record; the emotion taxonomy and ABSA evidence tags build on those labels; entity resolution maps text to tickers with sufficient precision for per-stock analysis; LIME is wired through an asynchronous REST endpoint to a frontend heatmap; and the correlation engine is accessible in the UI with appropriate statistical caveats. These are delivered and integrated components: the GitHub repository, benchmark labels, and figure generation scripts are in version control and independently reproducible (see Section~\ref{sec:frontend_screenshots} and Appendix~\ref{app:screenshots}).
...
\begin{itemize}
    ...
    \item Report the ABSA-lite extraction results on the 150k+ corpus, characterising the distribution of discussed aspects across sources, tickers, and temporal windows.
...
The project's novelty has been shaped and corroborated by practitioners throughout (Appendix~\ref{app:interviews}), and the next step is to expose the delivered system to a wider audience of domain experts under realistic conditions. An opportunity has been arranged for the summer 2026 period to present the system and its findings to a room of senior practitioners from the finance and AI sectors, in a live-demonstration format followed by structured Q\&A.

The demonstration has three aims, each of which maps onto a gap that a purely academic evaluation cannot close. First, it stress-tests the system's user interface and narrative against practitioner expectations rather than researcher expectations; the interviews made clear that trust, source attribution, and explainability matter at least as much as raw classification accuracy, and these qualities are only really assessed when the system is used by people who carry those concerns professionally. Second, it provides a structured setting in which to gather feedback on the three novel contributions (the finance-emotion taxonomy, the ABSA-lite evidence layer, and the production-grade LIME integration), separating those that practitioners find genuinely useful from those that are academically interesting but operationally peripheral. Third, it creates a realistic route toward potential real-world adoption: several practitioners during the requirements phase advised an MVP-first commercialisation pathway, and a live demonstration to a relevant audience is the natural first step along that pathway.

Any feedback collected during the demonstration will be recorded, anonymised in line with the project's ethics classification, and used to prioritise the future-work items listed in the preceding section. The demonstration is external to the academic submission and its outcomes are not a basis on which this dissertation should be graded; it is described here because it is the concrete next stage of the project's broader trajectory, and because the interviews that informed the current system design will be complemented by a second, practitioner-facing validation loop over the summer.
...
If I were starting again, I would sort out the Twitter data situation much earlier. I lost several weeks to the API rate limit problem that eventually led to the multi-backend architecture. Designing that resilience from day one would have freed up time I could have spent deepening the evaluation or building the formal emotion taxonomy annotation study that the journal paper pathway requires.
