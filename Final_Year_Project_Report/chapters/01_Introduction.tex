...
trading volume, and create temporary mispricings that persist until information is
fully absorbed \citep{tetlock2007giving}. With the proliferation of digital
communication, social media and online communities have become primary channels
through which retail investors express and exchange market opinions. Reddit,
...
trust, a requirement made explicit by \citeauthor{doshivelez2017interpretable}
(\citeyear{doshivelez2017interpretable}) and reinforced in the context of
financial AI by \citeauthor{yeo2023xai} (\citeyear{yeo2023xai}), who observe
that most XAI work in finance remains confined to offline research tools rather
than deployed user interfaces.
...
This is a research artefact rather than a regulated financial product. The React interface was designed with production-quality engineering in mind (clear navigation, responsive charts, and explicit Methodology and Legal pages), so that the system could credibly serve as an investor analytics prototype or the basis for a future pilot study. Where this report refers to \emph{investor-facing} or \emph{production-style} qualities, it means product-minded design that could support user trials or a journal demonstration, not a claim of FCA authorisation, suitability advice, or commercial SLAs.
...
\begin{enumerate}
    ...
    using a resilient multi-backend ingestion architecture with rate-limit
...
No existing system in the reviewed literature integrates multi-source ingestion,
a finance-specific emotion taxonomy, lightweight ABSA, production-grade XAI,
and an interactive correlation engine within a single reproducible tool.
This project makes three primary novel contributions:
...
\begin{enumerate}
    ...
    confidence, scepticism, mixed) that is computed from FinBERT's softmax
    probabilities, normalised Shannon entropy, domain lexicons, and financial
    aspect priors in a single inference pass. The combination of these four signals
    within one architecture-agnostic scoring function has not been previously
    reported, and the module is deployed in production, annotating the entire
    ...
    product/AI, and macro/policy) that operates without any labelled training data
    and attaches evidence sentences to each stored record, providing traceable
    justification for the headline sentiment label.

    \item \textbf{Production-grade XAI integration.}
    \citeauthor{yeo2023xai} (\citeyear{yeo2023xai}) find that most XAI
    applications in finance remain confined to offline notebooks; the same
    observation applies to every financial sentiment system in the reviewed
    literature. I embedded LIME \citep{ribeiro2016lime} as a live FastAPI REST
    endpoint, resolved the async/CPU-blocking challenge via
    \texttt{run\_in\_executor}, and rendered token-level heatmaps directly within
    the React frontend, making model explanations accessible to end users without
    any code interaction. LIME diagnostics also produced a concrete finding:
    contextually neutral risk vocabulary systematically triggers negative
    misclassification, providing an actionable target for future fine-tuning.
