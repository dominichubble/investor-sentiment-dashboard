...
%% ── Arrows T1 → T2 (all sources feed the shared preprocessing entry point) ──
\draw[arr] (reddit.south)  -- ++(0,-0.45) -| (preproc.north);
\draw[arr] (twitter.south) -- ++(0,-0.45) -| (preproc.north);
\draw[arr] (newsapi.south) -- ++(0,-0.45) -| (preproc.north);
...
%% ── Arrows T3 → T4 ───────────────────────────────────────────────
\draw[arr] (entity.south) -- ++(0,-0.4) -| (api.north);
\draw[arr] (db.south)     -- ++(0,-0.4) -| (api.north);
...
%% Successful exact match: bypass fuzzy via a segment above the row (no crossing)
\draw[arr] (exact.north) -- ++(0,0.55)
           -| node[above, font=\scriptsize\itshape, pos=0.28]{match found} (ticker.north);
\end{tikzpicture}}
