...
\node[src] (twitter) at (4.0,  0.0) {X/Twitter\\[-2pt]\scriptsize (X API v2)};
...
I collect text from three sources (Reddit, X/Twitter, and financial news) through dedicated pipelines. Each pipeline handles its own rate limits and authentication. They all feed into a shared preprocessing and FinBERT inference layer, after which the finance-emotion taxonomy and ABSA module enrich each record. A stock entity resolution layer then maps free-text mentions to canonical ticker symbols. Processed records go into a cloud PostgreSQL database (Neon). A FastAPI backend serves aggregate statistics, per-ticker sentiment, LIME explanations, and correlation results to the React frontend. I automated daily collection using GitHub Actions so the corpus continues to grow without manual intervention.
...
I did not cut any planned objectives, but the scope grew in one way I had not anticipated. The correlation module grew. I originally planned a simple Pearson correlation display, but reading the literature made it clear that a single coefficient is insufficient for financial time-series. I therefore extended it to include Spearman, lag analysis, Granger causality, rolling correlation, and market adjustment. The Twitter ingestion also turned out to be more constrained than planned. The official X API v2 rate and quota limits (tightened during the 2023--2024 platform changes) were more restrictive within an undergraduate budget than I had initially anticipated. That constraint is discussed honestly in Section~\ref{sec:twitter_ingestion} and Section~\ref{sec:correlation_discussion}. Both issues were absorbed into existing sprints without changing the high-level Gantt.
...
Beyond the University's internal ethics process, I also sought an external professional perspective on the project's data-handling decisions. On 10 October 2025 I held a one-hour consultation with a Certified Data Ethics Professional working in the UK financial-services sector. I walked through the project's ingestion sources, the anonymisation pipeline, the retention policy, and the downstream uses of the derived sentiment scores. Three recommendations from that conversation are reflected in the final system: (i) the explicit restriction of X/Twitter collection to the official API rather than unofficial scrapers, so that collection sits cleanly inside the platform's terms of service; (ii) stripping author-identifying fields (handles, display names, geolocation) at ingestion rather than at storage, so that personally identifying information never enters the working database; and (iii) framing the dashboard's outputs as a decision-support signal rather than an investment recommendation, to avoid any implication of automated financial advice. The consultation did not constitute formal ethical approval, which remains the University's responsibility. It did, however, provide an independent sanity-check on the governance posture of the project and directly shaped the choices documented below.

\subsection{Platform Terms of Service and API Compliance}

Reddit data is collected using the Python Reddit API Wrapper (PRAW) with OAuth2 authentication, operating strictly within Reddit's published developer terms. Rate limits are respected through automatic pagination and exponential backoff handling. X/Twitter data is collected exclusively via the official X API v2 (Tweepy with a bearer token), which enforces the platform's documented v2 rate limits automatically. This is a deliberate decision to keep the pipeline aligned with the X terms of service. The system also supports a historical CSV ingestion mode to enable reproducible evaluation against a fixed, pre-collected dataset. NewsAPI data is retrieved through a registered developer account using the documented API interface.
...
\begin{itemize}
    \item Collect and preprocess text data from Reddit, X/Twitter, and financial news via documented official interfaces, with rate-limit handling and a historical CSV path for reproducible evaluation.
...
I chose FinBERT \citep{araci2019finbert} as the core classifier because it is domain-adaptively pre-trained on financial corpora. That gives it a well-documented advantage over general-purpose models on finance-specific benchmarks \citep{malo2014financialphrasebank}. The model is loaded from the \texttt{ProsusAI/finbert} checkpoint via the Hugging Face \texttt{transformers} library \citep{wolf2020transformers} and used in zero-shot inference mode (no further fine-tuning). It classifies text as \textit{positive}, \textit{neutral}, or \textit{negative} with associated probability scores. I chose to preserve the full probability distribution rather than just the label, because those scores feed directly into the emotion taxonomy and entropy calculation downstream.
...
\section{Novel: Explainability Integration}
...
I chose LIME for real-time API responses and reserved SHAP for offline analysis. The trade-off here is theoretical rigour (SHAP) versus practical latency (LIME). The literature on deployable XAI supports that decision \citep{yeo2023xai}. SHAP's computational cost would have made it impossible to serve interactively from a cloud instance with realistic resource constraints.

\section{Stock Entity Resolution}
\label{sec:entity_design}

To enable per-ticker analytics, a stock entity resolution pipeline links free-text mentions to canonical ticker symbols. It is factored into two cooperating components rather than a single monolithic cascade:

\begin{enumerate}
    \item \textbf{Candidate extraction (upstream).} The \texttt{CompanyExtractor} module runs spaCy's \texttt{en\_core\_web\_sm} pipeline over the cleaned text to produce candidate \texttt{ORG}, \texttt{PRODUCT}, and \texttt{GPE} spans, and in parallel tokenises for explicit cashtag matches (e.g., \texttt{\$AAPL}).
    \item \textbf{Resolution cascade (\texttt{EntityResolver}).} Each candidate string is then resolved against the curated stock database in three priority stages:
    \begin{enumerate}
        \item \textit{Exact cashtag match} (\texttt{\$AAPL}~$\rightarrow$~\texttt{AAPL});
        \item \textit{Exact company-name match} against the canonical name index;
        \item \textit{Fuzzy match} using the \texttt{FuzzyWuzzy} partial-ratio scorer with a configurable similarity threshold (default 0.85), allowing resolution of common abbreviations and partial names.
    \end{enumerate}
\end{enumerate}

This split keeps the NER step, which depends on a heavyweight spaCy pipeline, separate from the pure-string matching logic, which can be unit-tested in isolation. A curated blacklist applied inside the resolver prevents common false positives (``Fed'', ``SEC'', ``FBI'', ``White House'') from being incorrectly resolved to tickers.  The three-stage pipeline is summarised in Figure~\ref{fig:entity_resolution}.
