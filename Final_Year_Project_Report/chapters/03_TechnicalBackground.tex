...
The key insight is that any two positions in a sequence can attend directly to one
another. This removes the sequential bottleneck that made RNNs slow to train and prone
to forgetting over long contexts.  Figure~\ref{fig:transformer_encoder} shows the structure of a single encoder layer.
...
softmax saturates into near-zero gradients. This is a practical issue that appeared
...
\begin{equation}
  ...
  \label{eq:lime_obj}
\end{equation}
In this expression, $\mathcal{L}(f, g, \pi_{x^*})$ is the weighted prediction error
between $f$ and $g$ over perturbed samples near $x^*$, and $\Omega(g)$ penalises the
complexity of $g$. The term $\pi_{x^*}(z) = \exp(-D(x^*, z)^2 / \sigma^2)$ is a proximity
kernel that gives more weight to samples close to $x^*$.
...
word means that word pushed the prediction toward the target class. A negative weight
means it pushed against it.
...
sensitive to retail commentary. For smaller or less liquid names the sentiment volume
is too low for the test to have meaningful statistical power.
...
explaining individual predictions, with different cost and fidelity trade-offs. Pearson,
Spearman, rolling correlation, and the Granger F-test together give a layered view of the
sentiment-price relationship that no single measure could provide alone.
...
interpret cases where the token heatmap contradicted my intuition. In several such cases
the disagreement was not a bug in my code, but a consequence of the linear surrogate
breaking down in a non-linear region of the decision boundary.
